...
class State:
    ...
    def __init__(self, board, predecessor, move):
        """ a constructor for a State object that initializes
            four attributes: board, predecessor, move, num_moves
        """
        self.board = board
    ...
    def generate_successors(self):
        """ creates and returns a list of State objects for 
            all successor states of the called State object.
        """
        successors = []
        
        # for loop to test all moves
        for m in MOVES:
            copy = self.board.copy()
            if copy.move_blank(m) == True:
                new = State(copy, self, m)
                successors.append(new)
        return successors


    def print_moves_to(self):
        """ prints the sequence of moves that lead from 
